function [x,y,ind,xx_yy] = spm_MDP_VB_ERP_RSS(MDP,FACTOR,T)
...
subplot(4,1,3), image(t,1:(size(v,2)),64*(1 - v')), ylabel('Unit')
title(sprintf('Unit reponses : %s',factor),'FontSize',16)
if numel(factor) < 16
    grid on, set(gca,'YTick',1:numel(name))
    set(gca,'YTickLabel',name)
...
%--------------------------------------------------------------------------
% subplot(6,1,3), plot(t,x',t,y','-.')
% title('Local field potentials','FontSize',16)
% ylabel('Depolarisation'),spm_axis tight
% grid on, set(gca,'XTick',(1:(length(t)/Nb))*Nb*dt)

% event related responses summed
%--------------------------------------------------------------------------

subplot(4,1,2), plot(t,xx(end,:))
title('Local field potentials (Level 2)','FontSize',16)
ylabel('Depolarisation'),ylim([-.4 .4])
% grid on, set(gca,'XTick',(1:(length(t)/Nb))*Nb*dt)

subplot(4,1,4), plot(t,yy(end,:))
title('Local field potentials (Level 1)','FontSize',16)
ylabel('Depolarisation'),ylim([-2 2])
grid on, set(gca,'XTick',(1:(length(t)/Nb))*Nb*dt)
